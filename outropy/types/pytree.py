...
from typing import OrderedDict as GenericOrderedDict
from typing import Protocol, Sequence, Tuple, Type, TypeVar, Union, cast, overload
...
class KeyEntry(Protocol):
    ...
    def delete(self, parent: Any) -> None: ...
...
DumpableContext = Any  # Any json dumpable plaintext
    ...
    *,
    ...
    Args:
        ...
        serialized_type_name: A keyword argument used to specify the fully qualified
            name used when serializing the tree call_graph.
...
class SequenceKey(Generic[T]):
    ...
    def delete(self, sequence: List[T]) -> None:
        del sequence[self.idx]
...
class MappingKey(Generic[K, T]):
    ...
    def delete(self, mapping: Dict[K, T]) -> None:
...
def _is_namedtuple_instance(tree: Any) -> bool:
    ...
    if not isinstance(fields, tuple):
        return False
    return all(type(entry) == str for entry in fields)  # noqa: E721
    ...
    def unflatten(self, leaves: Iterable[Any]) -> PyTree:
        ...
        if len(leaves) != self.num_leaves:
            raise ValueError(
                ...
                f"but the call_graph refers to a pytree that holds {self.num_leaves} "
                f"items ({self}).",
            )
        if self.is_leaf():
            return leaves[0]
...
#
# For example, given pytree=0 and call_graph=TreeSpec(list, None, [LeafSpec(), LeafSpec()]),
...
def _broadcast_to_and_flatten(
    ...
    # Check if the Node is different from the call_graph
...
def str_to_pytree(json_str: str) -> TreeSpec:
    return treespec_loads(json_str)
...
def key_del(obj: Any, kp: KeyPath) -> None:
    """Given an object and a key path, delete the value at the key path."""
    for k in kp[:-1]:
        obj = k.get(obj)
    kp[-1].delete(obj)
