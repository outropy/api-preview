...
async def run(outropy: OutropyApi, *args: Any) -> None:
    guidelines_ingestion_task = await outropy.create_task(
        task=Tasks.INGEST_REFERENCE_DATA,
        name="evaluation_criteria_v3",
        prompt="These are guidelines for evaluating code changes in a pull request.",
    )

    with console.status("Calling Outropy API to ingest the guidelines"):
        uploaded_data = await outropy.upload_text("evaluation_criteria_v3", mime_type="text/csv", text=evaluation_criteria)
        gudelines_datasource = await outropy.execute_and_wait_for_results(guidelines_ingestion_task, subject=[uploaded_data])
    ...
    code_review_task = await outropy.create_task(
        ...
        input_type=Diff,
        reference_data=[gudelines_datasource],
        examples=[example],
    )

    sample_input = await outropy.upload_object(name="code_diff_for_v4", obj=Diff(diff_output=diff_data))
